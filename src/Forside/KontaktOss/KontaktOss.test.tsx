...
describe("KontaktOss", () => {
	...
	it("Skal vise riktig tekst", () => {
		...
		expect(screen.getByRole("heading", { name: "Kontakt oss", level: 2 })).toBeInTheDocument();
	...
	it("Lenker går til riktig sted", () => {
		render(<KontaktOss kontaktOssUrl="http://test/" />);
		expect(screen.getByRole("link", { name: "55 55 33 36" })).toHaveAttribute("href", "tel:+4755553336");
		expect(screen.getByRole("link", { name: "Kontaktskjema" })).toHaveAttribute("href", KONTAKTSKJEMA_URL);
		expect(screen.getByRole("link", { name: "Kontakt oss" })).toHaveAttribute("href", "http://test/");
	});
});