...
export const tekniskeLoggFeil = {
  manglendeEllerMalformetMelding:
    "Prøver å logge en malformet eller manglende loggmelding",
...
const loggingHandler = (req: NextApiRequest, res: NextApiResponse): void => {
  ...
  const label: unknown = Object.values(tekniskeLoggFeil).includes(message)
    ? levels.error
    : level.label;
