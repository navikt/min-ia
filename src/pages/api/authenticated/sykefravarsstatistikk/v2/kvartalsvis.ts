...
export default async function handler(
    ...
    const erIProd = !(process.env.NAIS_CLUSTER === "dev-gcp" || process.env.NODE_ENV === "development");
    if (erIProd) {
        backendLogger.warn(`Endepunkt /kvartalsvis er ikke tilgjengelig i cluster '${process.env.NAIS_CLUSTER}' eller NODE_ENV '${process.env.NODE_ENV}'`);
