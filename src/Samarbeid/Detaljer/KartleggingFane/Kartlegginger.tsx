...
export default function Kartlegginger({
  dokumenter,
}: {
  ...
  if (
    !dokumenter.some(
      (d) => d.type === "BEHOVSVURDERING" || d.type === "EVALUERING",
    )
  ) {
  ...
  return dokumenter
    .filter(
      (dokument) =>
        dokument.type === "BEHOVSVURDERING" || dokument.type === "EVALUERING",
    )
    .sort(
      ({ dato: opprettetA }, { dato: opprettetB }) =>
        new Date(opprettetB).valueOf() - new Date(opprettetA).valueOf(),
    )
    .map((dokument) => (
