...
import {
  fiaSamarbeidDokumentMock,
  fiaSamarbeidMock,
} from "../../../local/fia-samarbeidMock";
import { axe } from "jest-axe";
import { useFiaDokument } from "../../fiaSamarbeidDokumenterAPI";
import {
  SvaralternativResultat,
  TemaMedSpørsmålOgSvar,
} from "../../../komponenter/Spørreundersøkelsesresultat/SpørreundersøkelseRad";
...
describe("KartleggingFane", () => {
  ...
  it("Render uten å krasje", () => {
    const { container } = render(<KartleggingFane />);
  ...
  it("Rendrer alle spørsmål", async () => {
    ...
    for (const spørsmål of JSON.parse(
      fiaSamarbeidDokumentMock("ba7d8dc5-b363-421b-9773-7e3c2185fa86").innhold,
    ).spørsmålMedSvarPerTema.flatMap(
      (t: TemaMedSpørsmålOgSvar) => t.spørsmålMedSvar,
    )) {
  ...
  it("Rendrer tomme grafer når ingen har svart", async () => {
    ...
    expect(
      await screen.findAllByText("For få deltakere til å vise resultater"),
    ).toHaveLength(2);

    // Sjekk at tittel på uferdige spørsmål også vises
    for (const spørsmål of JSON.parse(
      fiaSamarbeidDokumentMock("ba7d8dc5-b363-421b-9773-7e3c2185fa87").innhold,
    )
      .spørsmålMedSvarPerTema.flatMap(
        (t: TemaMedSpørsmålOgSvar) => t.spørsmålMedSvar,
      )
      .filter((s: { svarListe: SvaralternativResultat[] }) =>
        s.svarListe.every(
          (svar: { antallSvar: number }) => svar.antallSvar === 0,
        ),
      )) {
      expect(
        await screen.findAllByRole("heading", { name: spørsmål.tekst }),
      ).toHaveLength(1);
    }
  });
  ...
  it("kaller useFiaDokuement med riktig dokumentId", async () => {
    ...
    act(() => rader[0].click());
    await waitFor(() => expect(useFiaDokument).toHaveBeenCalledTimes(1));
    expect(useFiaDokument).toHaveBeenNthCalledWith(1, {
      dokumentId: "ba7d8dc5-b363-421b-9773-7e3c2185fa86",
    });
    act(() => rader[1].click());
    await waitFor(() => expect(useFiaDokument).toHaveBeenCalledTimes(2));
    expect(useFiaDokument).toHaveBeenNthCalledWith(2, {
      dokumentId: "c1a2b3d4-e5f6-7890-abcd-ef0123456789",
    });
  });
  ...
  it("Viser For få deltakere til å vise resultater når ingen har svart på grafer i halvferdige tema", async () => {
    ...
    const dokumentinnhold = JSON.parse(
      fiaSamarbeidDokumentMock("ce282c64-7aa8-4577-b161-088c405aa3b5").innhold,
    );
    render(<KartleggingFane />);
    expect(useFiaDokument).toHaveBeenCalledTimes(0);
    const rader = await screen.findAllByRole("button", { name: /Vis mer/ });
    ...
    // Sjekk at tittel på uferdige spørsmål også vises
    for (const spørsmål of dokumentinnhold.spørsmålMedSvarPerTema
      .flatMap((t: TemaMedSpørsmålOgSvar) => t.spørsmålMedSvar)
      .filter((s: { svarListe: SvaralternativResultat[] }) =>
        s.svarListe.every(
          (svar: { antallSvar: number }) => svar.antallSvar === 0,
  ...
  it("Sender analyttics ved åpning av rad", async () => {
    ...
    await waitFor(() =>
      expect(sendPanelEkspanderEvent).toHaveBeenCalledTimes(1),
    );

    expect(sendPanelEkspanderEvent).toHaveBeenNthCalledWith(
      1,
      "Behovsvurdering",
    );
    ...
    await waitFor(() =>
      expect(sendPanelEkspanderEvent).toHaveBeenCalledTimes(2),
    );
    expect(sendPanelEkspanderEvent).toHaveBeenNthCalledWith(
      2,
      "Behovsvurdering",
  ...
  it("Viser melding når ingen kartlegginger er tilgjengelig", () => {
    ...
    render(<KartleggingFane />);
    expect(
      screen.getByText("Ingen kartlegginger er publisert enda"),
    ).toBeInTheDocument();
  });
  ...
  it("Viser innhold for evalueringer når slike finnes", async () => {
    ...
    const forventetSpørsmålssett = JSON.parse(
      fiaSamarbeidDokumentMock("ba7d8dc5-b363-421b-9773-7e3c2185fa88").innhold,
    ).spørsmålMedSvarPerTema.flatMap(
      (t: TemaMedSpørsmålOgSvar) => t.spørsmålMedSvar,
    );
  ...
  it("Sorterer nyeste kartlegginger først", () => {
    ...
    expect(
      screen
        .getByText("01.12.2023")
        .compareDocumentPosition(screen.getByText("01.06.2023")),
    ).toBe(Node.DOCUMENT_POSITION_FOLLOWING);
    expect(
      screen
        .getByText("01.06.2023")
        .compareDocumentPosition(screen.getByText("01.01.2023")),
    ).toBe(Node.DOCUMENT_POSITION_FOLLOWING);
  });
