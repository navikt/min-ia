...
describe("PlanFane", () => {
  ...
  it("skal rendre PlanFane", async () => {
    const { container } = render(<PlanFane />);
    expect(container).toBeInTheDocument();

    expect(screen.getByText("Partssamarbeid")).toBeInTheDocument();

    const utviklePartssamarbeidet = await screen.findAllByText(
    ...
    expect(
      within(utviklePartssamarbeidet[0].parentElement!).getByTestId(
        "i-dag-pin",
      ),
    ).toBeInTheDocument();
    expect(
  ...
  it("Sender analytics ved åpning av undertema", async () => {
    const { container } = render(<PlanFane />);
    expect(container).toBeInTheDocument();
    const rader = await screen.findAllByRole("button", {
      name: /Utvikle partssamarbeidet/,
    });
