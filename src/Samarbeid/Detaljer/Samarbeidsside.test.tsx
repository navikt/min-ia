...
describe("Samarbeidsside", () => {
  ...
  it("Render uten å krasje", () => {
    ...
    expect(
      screen.getByRole("tab", { name: "Kartlegginger" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("tab", { name: "Samarbeidsplan" }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("heading", { name: mockdata[0].navn }),
    ).toBeInTheDocument();
  });
  ...
  it("Defaulter til kartlegginger når det ikke finnes plan", () => {
    const samarbeidUtenPlan = {
      ...mockdata[0],
      dokumenter: mockdata[0].dokumenter?.filter(
        (d) => d.type !== "SAMARBEIDSPLAN",
      ),
    };
    jest.mocked(useFiaSamarbeid).mockReturnValue({
      status: RestStatus.Suksess,
    ...
    render(
      <OrgnrProvider>
        <Samarbeidsside
          samarbeidOffentligId={samarbeidUtenPlan.offentligId}
          setSamarbeidOffentligId={() => {}}
        />
      </OrgnrProvider>,
    );

    expect(screen.getByRole("tab", { name: "Kartlegginger" })).toHaveAttribute(
      "aria-selected",
      "true",
    );
    expect(screen.getByRole("tab", { name: "Samarbeidsplan" })).toHaveAttribute(
      "aria-selected",
      "false",
    );
  });
  ...
  it("Viser riktig status på samarbeid", () => {
    ...
    const { container, rerender } = render(
      <OrgnrProvider>
        <Samarbeidsside
          samarbeidOffentligId={førsteFullførte.offentligId}
          setSamarbeidOffentligId={() => {}}
        />
      </OrgnrProvider>,
    );
    ...
    expect(
      screen.getByText(
        penskrivIAStatus(førsteFullførte.status as SamarbeidStatus),
      ),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(
        penskrivIAStatus(førsteAktive.status as SamarbeidStatus),
      ),
    ...
    rerender(
      <OrgnrProvider>
        <Samarbeidsside
          samarbeidOffentligId={førsteAktive.offentligId}
          setSamarbeidOffentligId={() => {}}
        />
      </OrgnrProvider>,
    );
  ...
  it("Håndterer feil ved lasting av samarbeid", () => {
    (
      jest.requireMock("../fiaSamarbeidAPI").useFiaSamarbeid as jest.Mock
    ).mockReturnValue({
    ...
    expect(
      screen.getByText("Noe gikk galt ved lasting av samarbeid"),
    ).toBeInTheDocument();
  });
  ...
  it("Håndterer ingen samarbeid", () => {
    ...
    render(
      <Samarbeidsside
        samarbeidOffentligId={undefined}
        setSamarbeidOffentligId={() => {}}
      />,
    );
    expect(
      screen.getByText("Denne virksomheten har ingen aktive samarbeid"),
    ).toBeInTheDocument();
  });
  ...
  it("Sender metrikk ved bytte av tab", () => {
    ...
    const samarbeidsplanTab = screen.getByRole("tab", {
      name: "Samarbeidsplan",
    });
