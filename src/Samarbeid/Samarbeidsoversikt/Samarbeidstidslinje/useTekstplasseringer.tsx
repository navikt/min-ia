...
export function useTekstplasseringer(transformerteHendelser: { prosent: number; hendelse: Samarbeidhendelse; }[], tidslinjeBredde: number): Array<{
	start: number;
	end: number;
	width: number;
}> {
	return React.useMemo(() => {
		const tekstplasseringer: Array<{ start: number; end: number; width: number; }> = [];
		...
		for (let i = 0; i < transformerteHendelser.length; i++) {
			...
			const width = getTextWidth(`${beskrivelse.boldText}: ${beskrivelse.normalText}`);
			...
			} else if (prosent * tidslinjeBredde / 100 > (tidslinjeBredde - (width / 2))) {
				tekstplasseringer.push({
...
function getTextWidth(text: string) {
	...
	context.font = getComputedStyle(document.body).font.replace(/\d\dpx/g, "14px");
