...
export default function Samarbeidstidslinje({
  hendelser,
}: {
      ...
      hendelser.reduce(
        (acc, hendelse) => {
          ...
          if (hendelse.type === "SAMARBEIDSPLAN") {
            ...
            if (
              !acc.førsteHendelse ||
              hendelse.start < acc.førsteHendelse.dato
            ) {
...
function Tidslinje({
  ...
  const { planStartProsent, planSluttProsent, tidMellomStartOgSlutt } =
    React.useMemo(() => {
      if (!førsteHendelse || !sisteHendelse) {
        return {
          planStartProsent: 0,
          planSluttProsent: 0,
          tidMellomStartOgSlutt: 0,
        };
      }

      const tidMellom =
        sisteHendelse.dato.getTime() - førsteHendelse.dato.getTime();

      const startProsent = planStart
        ? ((planStart.getTime() - førsteHendelse.dato.getTime()) / tidMellom) *
          100
        : 0;

      const sluttProsent = planSlutt
        ? ((planSlutt.getTime() - førsteHendelse.dato.getTime()) / tidMellom) *
          100
        : 0;
  ...
  const transformerteHendelser = React.useMemo(() => {
    ...
    return ikkePlanHendelser.map((hendelse) => ({
      prosent:
        ((hendelse.dato.getTime() - førsteHendelse.dato.getTime()) /
          tidMellomStartOgSlutt) *
        100,
      hendelse,
    }));
  ...
  const tekstplasseringer = useTekstplasseringer(
    transformerteHendelser,
    tidslinjeBredde,
  );
      ...
      <div className={styles.tidslinje} ref={tidslinjeRef}>
        {planStart && planSlutt && (
          <>
            <BodyShort
              size="small"
              className={styles.planbeskrivelse}
              style={{ left: `${planStartProsent}%` }}
            >
              <b>Aktiviteter i samarbeidsplan</b>{" "}
              {planStart.toLocaleDateString("nb-NO")} -{" "}
              {planSlutt.toLocaleDateString("nb-NO")}
            </BodyShort>
            <div
              className={styles.planlinje}
              style={{
                left: `${planStartProsent}%`,
                right: `${100 - planSluttProsent}%`,
              }}
            />
          </>
        )}
...
function TidslinjeHendelse({
  ...
  const avstandDisplayTextStyle = React.useMemo(() => {
    if (
      index > 0 &&
      tekstplasseringer[index - 1].end > tekstplasseringer[index].start
    ) {
      return { display: "none" };
    }
  ...
  const textStyle = React.useMemo(() => {
    ...
    if (
      ...
      tekstplasseringer[index].width / 2
    ) {
      // Very close to left edge - align text to the left
      return { left: "0", transform: "none" };
    } else if (
      (prosent * tidslinjeBredde) / 100 >
      tidslinjeBredde - tekstplasseringer[index].width / 2
    ) {
  ...
  return (
    <div className={styles.hendelse} style={{ left: `${prosent}%` }}>
      <Hendelsebeskrivelse
        hendelse={hendelse}
        textStyle={{ ...textStyle, ...avstandDisplayTextStyle }}
      />
    </div>
  );
}

function Hendelsebeskrivelse({
  hendelse,
  textStyle,
}: {
  hendelse: Samarbeidhendelse;
...
export function hendelseBeskrivelseTekst(hendelse: Samarbeidhendelse): {
  ...
  switch (hendelse.type) {
    case "SAMARBEID_STATUSENDRING":
      return {
        boldText: penskrivIAStatus(hendelse.nyStatus),
        normalText: hendelse.dato.toLocaleDateString("nb-NO"),
      };
    case "SAMARBEIDSPLAN":
      return {
        boldText: "Samarbeidsplan",
        normalText: hendelse.dato.toLocaleDateString("nb-NO"),
      };
    case "BEHOVSVURDERING":
      return {
        boldText: "Behovsvurdering",
        normalText: hendelse.dato.toLocaleDateString("nb-NO"),
      };
    case "EVALUERING":
      return {
