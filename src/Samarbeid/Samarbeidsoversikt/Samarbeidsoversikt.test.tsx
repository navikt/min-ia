...
describe("Samarbeidsvelger", () => {
  ...
  it("Render uten å krasje", () => {
    const { container } = render(<Samarbeidsoversikt />);
  ...
  it('Får "Vis alle" knapp når flere enn 3 samarbeid', async () => {
    ...
    expect(container).toBeInTheDocument();
    expect(
      screen.getAllByRole("button", { name: "Se samarbeid" }),
    ).toHaveLength(3);

    const seAlleKnapp = screen.getByRole("button", {
      name: `Vis alle (${mockdata.length})`,
    });
  ...
  it("Viser bare aktive samarbeid over fold", async () => {
    (useFiaSamarbeid as jest.Mock).mockImplementationOnce(() => ({
      status: RestStatus.Suksess,
      data: mockdata.map((v, i) => ({
        ...v,
        status: i < 2 ? "AKTIV" : "FULLFØRT",
      })),
    }));
  ...
  it("Autofokuserer på første etter expand", async () => {
    ...
    const filtrerteSamarbeid = mockdata
      .filter(({ status }) => status === "AKTIV")
      .slice(0, DEFAULT_MAKS_VISIBLE_SAMARBEID);

    const seSamarbeidKnapper = screen.getAllByRole("button", {
      name: "Se samarbeid",
    });
    const samarbeidSomIkkeVarSynlig = seSamarbeidKnapper.filter((knapp) =>
      filtrerteSamarbeid.every(
        (s) => `/samarbeid/${s.offentligId}` !== knapp.getAttribute("href"),
      ),
    );
    expect(samarbeidSomIkkeVarSynlig).toHaveLength(
      mockdata.length - filtrerteSamarbeid.length,
    );
  ...
  it("Viser samlet rad for inaktive samarbeid når ingen aktive", async () => {
    ...
    expect(
      screen.getByText(`Avsluttede samarbeid (${mockdata.length})`),
    ).toBeInTheDocument();
  });
  ...
  it("Sorterer aktive samarbeid først", async () => {
    ...
    expect(seSamarbeidKnapper[0].getAttribute("href")).toBe(
      `/samarbeid/${mockdata[1].offentligId}`,
    );
    expect(seSamarbeidKnapper[1].getAttribute("href")).toBe(
      `/samarbeid/${mockdata[2].offentligId}`,
    );
    expect(seSamarbeidKnapper[2].getAttribute("href")).toBe(
      `/samarbeid/${mockdata[0].offentligId}`,
    );
  });
  ...
  it("Sorterer basert på dokumentdato når nyere enn sistEndret", async () => {
    ...
    expect(seSamarbeidKnapper[0].getAttribute("href")).toBe(
      `/samarbeid/${mockdata[0].offentligId}`,
    ); // dokumentdato 2024-05-05
    expect(seSamarbeidKnapper[1].getAttribute("href")).toBe(
      `/samarbeid/${mockdata[3].offentligId}`,
    ); // sistEndret 2024-04-04
    expect(seSamarbeidKnapper[2].getAttribute("href")).toBe(
      `/samarbeid/${mockdata[2].offentligId}`,
    ); // sistEndret 2024-03-03
    expect(seSamarbeidKnapper[3].getAttribute("href")).toBe(
      `/samarbeid/${mockdata[1].offentligId}`,
    ); // sistEndret 2024-02-02
  });
