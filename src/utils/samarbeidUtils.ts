...
export function sorterSamarbeidsliste(samarbeidsliste: FiaSamarbeidDto[]): FiaSamarbeidDto[] {
	return samarbeidsliste.sort((a, b) => {
		...
		const aSisteDato = Math.max(...a.dokumenter.map(d => new Date(d.dato).getTime()), new Date(a.sistEndret).getTime());
		const bSisteDato = Math.max(...b.dokumenter.map(d => new Date(d.dato).getTime()), new Date(b.sistEndret).getTime());
