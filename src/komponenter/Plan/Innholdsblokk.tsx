  ...
  return (
    <Accordion className={styles.styledAccordion}>
      ...
      {filtrerteUndertemaer.map((undertema) => (
        <Innholdsrad key={undertema.id} undertema={undertema} />
      ))}
    </Accordion>
  );
}
...
function Innholdsrad({
  ...
  undertema: PlanTema["undertemaer"][number];
}) {
  return (
    <Accordion.Item
      className={styles.styledAccordionItem}
      onOpenChange={(open) => {
        if (open) {
    ...
    >
      <Accordion.Header className={styles.styledAccordionHeader}>
        ...
        >
          ...
          {undertema.sluttDato && (
            <PrettyInnholdsDato date={undertema.sluttDato} />
          )}
        </span>
        <PrettyPrintUndertemaStatus status={undertema.status} />
      </Accordion.Header>
      <Accordion.Content className={styles.styledAccordionContent}>
        <BodyLong>{undertema.målsetning}</BodyLong>
      </Accordion.Content>
    </Accordion.Item>
  );
}

function PrettyPrintUndertemaStatus({
  status,
}: {
