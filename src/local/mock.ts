...
export default async function mockRequest(req: NextRequest) {
  ...
  if (req.nextUrl.pathname.includes("/api/authenticated/organisasjoner")) {
    console.log(
      `[DEBUG] GET ${req.nextUrl.pathname}  - w/ params: ${req.nextUrl.searchParams}`,
    );
  ...
  if (req.url?.endsWith("/organisasjoner-med-statistikktilgang")) {
    console.log("[DEBUG] GET /api/[...]/organisasjoner-med-statistikktilgang");
  ...
  if (
    req.nextUrl.pathname.includes(
      "/api/sykefravarsstatistikk-api/kvartalsvis-sykefravarshistorikk",
    ) ||
    req.nextUrl.pathname.includes(
      "/api/authenticated/sykefravarsstatistikk/v2/kvartalsvis",
    )
  ) {
    console.log(
      `[DEBUG] GET ${req.nextUrl.pathname}  - w/ params: ${req.nextUrl.searchParams}`,
  ...
  if (
    req.nextUrl.pathname.includes(
      "/api/sykefravarsstatistikk-api/publiseringsdato",
    ) ||
    req.nextUrl.pathname.includes(
      "/api/authenticated/sykefravarsstatistikk/v2/publiseringsdato",
    )
  ) {
    console.log(
      `[DEBUG] GET ${req.nextUrl.pathname}  - w/ params: ${req.nextUrl.searchParams}`,
  ...
  if (req.url?.includes("api/authenticated/fia-dokument")) {
    ...
    return new NextResponse(
      JSON.stringify(fiaSamarbeidDokumentMock(dokumentId)),
      {
        status: 200,
      },
    );
  ...
  if (req.url?.includes("api/authenticated/fia-samarbeid?orgnr=")) {
    const orgnr = req.nextUrl.searchParams.get("orgnr") as string;
    console.log(`[DEBUG] GET /api/authenticated/fia-samarbeid?orgnr=${orgnr}`);

    if (orgnr === "999999997") {
      return new NextResponse(
        JSON.stringify({ message: "mock_feilmelding..." }),
        { status: 500 },
      );
    }
    ...
    if (orgnr === "999999996") {
      return new NextResponse(
        JSON.stringify({
          message: "Har ikke tilgang til resurs for orgnummer",
        }),
        { status: 403 },
      );
    }
